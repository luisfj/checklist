...
public class ListWorkersFromProjectBySlugControllerTest extends ChecklistApplicationBaseTest {
    ...
    void start() {
        ...
        Worker worker2 = workerRepository.createWorker(new Worker(WORKER_SLUG_OTHER, WORKER_SLUG_OTHER, project))
                .block();
    ...
    void testWithExistingProjectSlug_thenReturnWorkersFromProject() {
        webTestClient
                ...
                .expectBody(WorkersFromProjectResponse[].class).value(list -> {
                    assertTrue(responseExpected.size() > 0 && responseExpected.size() == list.length
                            && responseExpected.containsAll(Arrays.asList(list)));
                });
    }
